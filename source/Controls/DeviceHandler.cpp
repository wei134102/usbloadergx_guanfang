...
bool DeviceHandler::IsMounted(int dev)
{
	if (dev == SD)
...
bool DeviceHandler::MountSD()
{
	...
	int partCount = sd->GetPartitionCount();
	if (partCount < 1)
	{
		delete sd;
		sd = NULL;
	...
	{
		if (strncmp(sd->GetFSName(i), "FAT", 3) == 0)
		{
			if (sd->Mount(i, DeviceName[SD]))
				return true;
		}
	}
...
const char *DeviceHandler::GetFSName(int dev)
{
	...
	{
		int sdPartition = DeviceHandler::GetSDPartition();
		return DeviceHandler::instance->sd->GetFSName(sdPartition);
	}
	else if (dev >= USB1 && dev <= USB8)
	{
		int partCount0 = 0;
...
int DeviceHandler::GetSDPartition()
{
	if (!instance)
		return 0;

	for (int i = 0; i < instance->sd->GetPartitionCount(); ++i)
	{
		// Use the first FAT32 partition
		if (strncmp(instance->sd->GetFSName(i), "FAT", 3) == 0)
			return i;
	}
