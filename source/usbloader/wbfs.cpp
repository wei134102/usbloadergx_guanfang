...
s32 WBFS_OpenPartSD()
{
	int partition = DeviceHandler::GetSDPartition();

	if (WBFS_OpenPart(partition) == 0)
	{
		gprintf("WBFS_OpenPartSD partition: %d\n", partition);
...
s32 WBFS_OpenPart(int part_num)
{
	...
	int portPart = Settings.SDMode ? part_num : DeviceHandler::PartitionToPortPartition(part_num);
	int usbPort = Settings.SDMode ? 0 : DeviceHandler::PartitionToUSBPort(part_num);

	// if(handle->IsActive(part_num))
